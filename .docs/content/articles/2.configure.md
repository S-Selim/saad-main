...
date: 2022-08-23
description: Learn how to configure Saad with the app.config.ts file.
layout: article
---

# Configure Saad

To configure meta tags, social links or even the Saad theme display, update the `alpine` key in the `app.config.ts` at the root of your project:

```ts [app.config.ts]
export default defineAppConfig({
  alpine: {
    /* Saad configuration goes here */
  }
}
```
...
Saad Header and Footer can also be customized via the `app.config.ts` file:
